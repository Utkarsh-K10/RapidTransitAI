...
for file in os.listdir(employee_pictures):
	...
	# print(employee)
	img_path = '/Users/utkarshkushwaha/Downloads/ITDEPT/testproject/database/{}.jpg'.format(employee)
	impath = img_path
	print(impath)
	img = detectFace(impath)
...
while(True):
	...
	for (x,y,w,h) in faces:
		if w > 130: #discard small detected faces
			...
			# print("The original coordinates are..",(x,y), (x+w,y+h))
			cv2.putText(img,'PUT YOUR FACE HERE',(460,122), cv2.FONT_HERSHEY_SIMPLEX, 1, (4, 265, 0), 2)
			cv2.rectangle(img, (460,122),(851,513),(255,0,0),2)
			...
			if is_found:
				# display_img = cv2.imread("/Users/utkarshkushwaha/Downloads/ITDEPT/testproject/database/%s.jpg" % employee_name)
				display_img = cv2.imread("/Users/utkarshkushwaha/Downloads/ITDEPT/testproject/database/%s.jpg" % employee_name)
				pivot_img_size = 112
					...
					if y - pivot_img_size > 0 and x + w + pivot_img_size < resolution_x:
						...
						if tname in lbls:
							...
							for itr, word in enumerate(detail[0]):
								offset += int(resolution_y/len(lbls))- 10
								cv2.putText(img, str(word),(20, offset), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 225, 104), 1)
						...
						if tname in lbls:
							...
							for itr, word in enumerate(detail[0]):
								offset += int(resolution_y/len(lbls))- 10
								cv2.putText(img, str(word),(20, offset), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 155, 0), 1)
				...
				display_img = cv2.imread("/Users/utkarshkushwaha/Downloads/ITDEPT/testproject/unknown.jpg")
				pivot_img_size = 112
				display_img = cv2.resize(display_img, (pivot_img_size, pivot_img_size))
				try:
					resolution_x = img.shape[1]; resolution_y = img.shape[0]
					...
					if y - pivot_img_size > 0 and x + w + pivot_img_size < resolution_x:
						#top right
						img[y - pivot_img_size:y, x+w:x+w+pivot_img_size] = display_img
						cv2.putText(label, (x+w, y+10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (67,67,67), 1)					
						...
						#bottom left
						img[y+h:y+h+pivot_img_size, x-pivot_img_size:x] = display_img
						cv2.putText(label, (x - pivot_img_size, y+h-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (67,67,67), 1)
						...
						#top left
						img[y-pivot_img_size:y, x-pivot_img_size:x] = display_img
						cv2.putText(label, (x - pivot_img_size, y+10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (67,67,67), 1)
						...
						#bottom righ
						img[y+h:y+h+pivot_img_size, x+w:x+w+pivot_img_size] = display_img
						cv2.putText(label, (x+w, y+h-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (67,67,67), 1)
